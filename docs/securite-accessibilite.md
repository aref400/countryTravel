...
| **A06 — Vulnerable and Outdated Components** | Dépendances majeures à jour (NestJS 11, Prisma 7), lockfile committé ; `npm audit fix` non destructif appliqué (vulnérabilité critique, modérées et basses résolues, sans changement de version majeure ni régression). Résiduel trié ci-dessous : 12 alertes (9 *high*, 3 *moderate*) issues de deux chaînes transitives — ReDoS `d3-color` via `react-simple-maps` (carte), et les CLI `prisma dev` / `shadcn` jamais exécutées en production — aucune atteignable à l'exécution | `apps/front/package.json`, `apps/api/package.json` |
